...
/**
 * NOT YET PORTED TO THE v3 WALL — see docs/PLAN.md.
 *
 * This harness earns its keep by summing the layout chain *independently* of
 * `useMapFrames`, so that a change to one has to be justified against the
 * other. v3 inverts that chain: the map used to be one scroller containing
 * every rack, and is now a rack containing its own scroller, with cells that
 * share the plank's width instead of having a fixed one.
 *
 * Re-deriving these offsets against the new implementation would be quick and
 * would defeat the point — the same hand writing both sides of a check makes
 * it a mirror, not a check. So it is skipped rather than adjusted or deleted,
 * and wants the eyes that wrote it.
 *
 * In the meantime the same chain is covered from the other side, at the level
 * below: `mapFrames.test.ts` asserts the sum link by link, and `mapDrag.test.ts`
 * plays whole gestures against it.
 */
describe.skip('dragging a bin across the map', () => {
