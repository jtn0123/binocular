...
import { CARD_H } from '@/components/map/metrics';

/**
 * The fixed-cell geometry this harness was written against, kept literal.
 *
 * v3 replaced it: slots now share the plank's width (`slotWidth`), so there
 * is no constant card width and no constant pitch. These stay here, spelled
 * out, so the harness still says exactly what layout it describes — and so
 * that re-pointing it at v3 is a deliberate edit rather than a silent
 * inheritance of whatever the constants happen to be now.
 */
const CARD_W = 118;
const SLOT_PITCH = CARD_W + 8;
